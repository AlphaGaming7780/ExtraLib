...
namespace ExtraDetailingTools.UI
{
    ...
    public partial class TransformSection : UISystemBase
    {
        ...
        private static void UpdateObjectTransform(float3? position, float3? rotation)
        {
            ...
            {
                ELT.Logger.Warn("Can't get the transform object.");
            ...
            {
                ELT.Logger.Warn("Failed to get the CullingInfo on the Entity");
                transformSectionGetPos.Update();
                transformSectionGetRot.Update();
                return;
            }
            ...
            if (ELT.m_EntityManager.TryGetBuffer(selectedEntity, false, out DynamicBuffer<InstalledUpgrade> installedUpgrades))
            {
