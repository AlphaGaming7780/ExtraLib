...
namespace ExtraDetailingTools.Patches
{
    ...
    {
        private static readonly string IconsResourceKey = $"{ELT.Logger.name.ToLower()}";
    ...
    class ToolUISystem_OnToolChanged
    {
        ...
        private static void Postfix(ToolBaseSystem tool)
        {
            ...
            if (tool is AreaToolSystem || tool is SurfaceReplacerTool)
            { //|| tool is ObjectToolSystem
    ...
    public class PrefabSystem_OnCreate
    {
        ...
        {
            ELT.Logger.Info("PrefabSystem_OnCreate");
    ...
    public class PrefabSystem_AddPrefab
    {
        ...
        public static bool Prefix(PrefabSystem __instance, PrefabBase prefab)
        {
            ...
            try
            {
                ...
                else if (prefab is SurfacePrefab surfacePrefab && spawnableArea != null)
                {
                    ...
                    if (setupCustomSurfaces)
                    {
                        ...
                        catch (Exception e) { ELT.Logger.Warn(e); }
                    }
                ...
                if (prefab is TerraformingPrefab) PrefabUI.m_Group = Prefab.GetExistingToolCategory(prefab, "Terraforming") ?? PrefabUI.m_Group;
                else if (prefab is SurfacePrefab) PrefabUI.m_Group ??= CustomSurfaces.SetupUIGroupe(prefab);
                else if (prefab.name.ToLower().Contains("decal") || prefab.name.ToLower().Contains("roadarrow") || prefab.name.ToLower().Contains("lanemarkings") || prefab.GetComponent<CustomDecal>() != null) PrefabUI.m_Group = CustomDecals.SetupUIGroupe(prefab);
                else PrefabUI.m_Group ??= Prefab.GetOrCreateNewToolCategory(prefab, "Landscaping", "[ELT] Failed Prefab, IF you see this tab, repport it, it's a bug.");

                ELT.Logger.Info(prefab);
            ...
            catch (Exception e) { ELT.Logger.Error(e); }
