...
public class CustomSurfaces
{
    ...
    private static void CreateCustomSurface(string folderPath, Material material, string CatName, string modName)
    {
        ...
        if (!File.Exists(folderPath + "\\" + "_BaseColorMap.png")) { ELT.Logger.Error($"No _BaseColorMap.png file for the {new DirectoryInfo(folderPath).Name} surface in {CatName} category in {modName}."); return; }
        ...
        try
        {
            ...
            if (!texture2D_BaseColorMap.LoadImage(fileData)) { ELT.Logger.Error($"[ELT] Failed to Load the BaseColorMap image for the {surfacePrefab.name} surface."); return; }
        ...
        catch (Exception e) { ELT.Logger.Warn(e); return; }
        ...
        // 	ELT.Logger.LogMessage(texture2D);
